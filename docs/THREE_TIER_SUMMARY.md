...
**OLD:** JUNO â†’ Ceres â†’ Process

**NEW:** NCSU â†’ JUNO â†’ Ceres â†’ Process
...
- `ncsu_sync_status TEXT` - Status of NCSU â†’ JUNO sync
...
# Syncing
./scripts/workflow.sh sync <BATCH_ID>        # Sync single batch NCSU â†’ JUNO
./scripts/workflow.sh sync-all [STATE]       # Sync all missing batches

# Complete pipeline
./scripts/workflow.sh full-pipeline <BATCH_ID>  # NCSU â†’ JUNO â†’ Ceres â†’ Process
...
- JUNO â†’ Ceres transfers work exactly as before
- Processing time unchanged
- Database operations remain fast

**New capabilities add:**
- NCSU â†’ JUNO sync: ~5-15 min per batch (one-time per batch)
