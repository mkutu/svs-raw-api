...
```
NCSU NFS ──sync──> JUNO LTS ──transfer──> Ceres /90daydata ──process──> Final Output
...
missing=$(./scripts/workflow.sh check-missing | grep "• " | awk '{print $2}')
...
echo "Ceres:"
ls -1 /90daydata/dash_agir/data/semifield-upload/$batch/*.ARW | wc -l
```
...
|-------|-------------------|-----------------|
| NCSU → JUNO sync | 5-15 min | 1 transfer |
| JUNO → Ceres transfer | 5-10 min | 1 transfer |
| RAW → DNG → JPG | 20-30 min | 12 SLURM jobs |
| **Total** | **40-60 min** | |
